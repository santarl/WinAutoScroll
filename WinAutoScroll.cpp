...
typedef struct {
    ...
    Shape dead_zone_shape; int dead_zone, cross_dead_zone_thickness;
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    switch (msg) {
        case WM_TRAYICON: if (lParam == WM_RBUTTONUP) ShowContextMenu(); break;
        case WM_COMMAND:
        ...
        case WM_APP_MBUTTON_DOWN:
            if (g_scrollState == STATE_IDLE) { g_scrollState = STATE_PRIMED; GetCursorPos(&g_primeStartPos); }
            else if (g_scrollState == STATE_SCROLLING && g_config.trigger_mode == MODE_TOGGLE) StopScrolling();
            break;
        case WM_APP_MBUTTON_UP:
            ...
            } else if (g_scrollState == STATE_SCROLLING && g_config.trigger_mode == MODE_HOLD) StopScrolling();
            break;
        case WM_APP_MOUSE_MOVE:
            if (g_scrollState == STATE_PRIMED) {
                POINT c; GetCursorPos(&c);
                if (abs(c.x - g_primeStartPos.x) > g_config.drag_threshold || abs(c.y - g_primeStartPos.y) > g_config.drag_threshold) StartScrolling();
            }
            break;
        case WM_APP_KEY_DOWN:
            if (g_config.trigger_mode == MODE_HOLD) StartScrolling();
            else { if (g_scrollState == STATE_IDLE) StartScrolling(); else if (g_scrollState == STATE_SCROLLING) StopScrolling(); }
            break;
...
void StartScrolling() {
    if (g_scrollState == STATE_IDLE || g_scrollState == STATE_PRIMED) {
        if(g_scrollState == STATE_IDLE) GetCursorPos(&g_startScrollPos); else g_startScrollPos = g_primeStartPos;
...
LRESULT CALLBACK LowLevelKeyboardProc(int nCode, WPARAM wParam, LPARAM lParam) {
    if (nCode == HC_ACTION && !g_isPaused) {
        ...
        if (wParam == WM_KEYDOWN && p->vkCode == VK_ESCAPE && (g_scrollState == STATE_SCROLLING || g_scrollState == STATE_PRIMED)) {
            PostMessage(g_hMainWnd, WM_APP_CANCEL, 0, 0); return 1;
        }
...
DWORD WINAPI ScrollingThread(LPVOID lpParameter) {
    while (g_scrollState == STATE_SCROLLING) {
        ...
        int dx = currentPos.x - g_startScrollPos.x, dy = currentPos.y - g_startScrollPos.y;
        int vS = 0, hS = 0;
        bool act = false;
...
void CreateOverlayWindow() {
    ...
    RegisterClassEx(&wc);
    g_hOverlayWnd = CreateWindowEx(WS_EX_LAYERED | WS_EX_TRANSPARENT | WS_EX_TOOLWINDOW | WS_EX_TOPMOST,
        "ScrollOverlay", NULL, WS_POPUP, 0, 0, 0, 0, NULL, NULL, g_hInstance, NULL);
}
...
void HideOverlay() { ShowWindow(g_hOverlayWnd, SW_HIDE); }
